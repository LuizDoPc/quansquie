...
<body onload="atualizaLista()">
    ...
    <div class="remodal" data-remodal-id="confirmation">
        ...
        <button data-remodal-action="confirm" class="remodal-confirm" id="apaga_tipo">Deletar</button>
    </div>
    ...
    <div class="remodal" data-remodal-id="modal">
        ...
        <div class="col s12 l4 offset-l4 center">
            <div class="flow-text">Inserir Tipos</div>
            <div class="row">
                <div class="input-field col s12 l6 offset-l3">
                    <input type="text" class="validate cadastro" id="nome_tipo" name="nome_tipo" maxlength="50">
                    <label for="nome_tipo">Nome</label>
                </div>
            </div>
        </div>
        <button data-remodal-action="cancel" class="remodal-cancel">Cancelar</button>
        <button data-remodal-action="confirm" class="remodal-confirm" id="envia_tipo">Enviar</button>
    </div>
    ...
    <div class="remodal" data-remodal-id="detalhes">
        ...
        <button class="remodal-confirm" id="atualiza_tipo"><i class="material-icons">create</i></button>
        <button class="remodal-confirm" id="envia_atualiza_tipo"><i class="material-icons">send</i></button>
    </div>
            ...
            <div class="col s12 l12">
                <div class="row center">
                    <div class="col s12 l4 offset-l4">
                        <div class="flow-text">Pesquisa Tipos</div>
                        <div class="input-field inline col s9 l8">
                            <input type="text" class="validae" id="pesquisa_tipo" name="pesquisa_tipo">
                            <label for="pesquisa_tipo">Pesquisa</label>
                        </div>
    ...
    <script>
        ...
        function atualizaLista(){
            $.get('../CONTROLE/Tipo/PesquisaTipo.php', {campo:''}, function (res) {
                $('#resposta_pesquisa').html(res);
            });
        }

        function enviaPesquisa() {
            $.get('../CONTROLE/Tipo/PesquisaTipo.php', {campo:$('#pesquisa_tipo').val()}, function (res) {
                $('#resposta_pesquisa').html(res);
            });
        }
        ...
        $(document).ready(function () {
            ...
            $('#envia_atualiza_tipo').hide();

            $(document).on('click', '#envia_tipo', function () {

                if(!$('#nome_tipo').val()){
                    alert('O campo Nome nao pode ser nulo.');
                }else
                $.get('../CONTROLE/Tipo/CadastraTipo.php', {nome:$('#nome_tipo').val()}, function () {
            ...
            $(document).on('click', '.resposta', function () {
                ...
                $.get('../CONTROLE/Tipo/RetornaTipo.php', {id:$(this).data('id')}, function (res) {
                    $('.resposta_modal').html(res);
                    inst.open();
                });
            });

            $(document).on('click', '#apaga_tipo', function () {
                var inst = $('[data-remodal-id=detalhes]').remodal();
                $.get('../CONTROLE/Tipo/ApagaTipo.php', {id:$('#Rid_tipo').val()}, function (res) {
                    atualizaLista();
                    inst.close();
            ...
            $(document).on('click', '#atualiza_tipo', function () {
                $('.editavel').attr('disabled', false);
                $(this).hide();
                $('#envia_atualiza_tipo').show();
            });

            $(document).on('click', '#envia_atualiza_tipo', function () {
                var inst = $('[data-remodal-id=detalhes]').remodal();
                if(!$('#Rnome_tipo').val()){
                    alert('O campo Nome nao pode ser nulo.');
                }else
                $.get('../CONTROLE/Tipo/AtualizaTipo.php', {id:$('#Rid_tipo').val(), nome:$('#Rnome_tipo').val()}, function () {
                    atualizaLista();
                    inst.close();
                });
            });
